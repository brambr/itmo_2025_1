...
public class CntrPassController {
    ...
    @Operation(summary = "Паспорт контейнера по ID", description = "Вывод по ID паспорта контейнера со всеми вхождениями")
    @GetMapping("/{id}")
    ...
    @Operation(summary = "Паспорт контейнера по номеру контейнера", description = "Вывод по номеру контейнера паспорта контейнера со всеми вхождениями")
    @GetMapping("/cntr/{number}")
    public CntrPassDto getCntrPassDtoByCntrNumber(@PathVariable String number) {
        CntrPass cntrPass = cntrPassService.findByCntrNumber(number);
        CntrPassDto cntrPassDto = mapper.map(cntrPass, CntrPassDto.class);
        return cntrPassDto;
    }
